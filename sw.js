...
//
// The whole site is taken at install: every day grid, every talk, every photo.
// It comes to about 5MB, which is a one-time cost on the way in and buys a
// programme that works in a basement conference room with no signal at all -
// including talks the visitor never happened to open while they still had it.
//
 ...
 * trade for eight shell files and the wrong one for five hundred. A single
 * photo 404ing must not cost the visitor every cached page. Whatever is missed
 * here still resolves through the fetch handler on first use.
 *
  ...
  const worker = async () => {
    while (next < REST.length) {
      ...
      try {
        ...
        /* offline mid-install: the fetch handler will pick this one up later */
      }
    }
  };
